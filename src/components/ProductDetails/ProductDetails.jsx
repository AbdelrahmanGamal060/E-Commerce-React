  ...
  return <>
    ...
    <div className="grid grid-cols-[1fr_2fr] gap-3 items-center">
      ...
      <div className="">
        ...
        <h3 className="mb-3 text-gray-400" >{ProductDetails?.description}</h3>
        <h3>{ProductDetails?.category.name}</h3>

        <div className="flex justify-between">
          {ProductDetails?.priceAfterDiscount ? <div className="flex gap-2">
            <h3 className="text-red-500 line-through">{ProductDetails?.price} EGP</h3>
            <h3>{ProductDetails?.priceAfterDiscount} EGP</h3>

          </div> : <h3>{ProductDetails?.price} EGP</h3>}
          <span> <i className="fas fa-star text-yellow-400"></i>  {ProductDetails?.ratingsAverage} </span>

        </div>
        <button  onClick={() => addToCartProduct(ProductDetails._id) }     className="   hover:bg-green-500 hover:text-white  border border-green-400 rounded-md w-full py-1.2 my-5 cursor-pointer transition-all duration-200">add To cart</button>

      </div>

    </div>

    <h1 className="text-center" >RelatedProduct</h1>
    <div className=" grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
      {relatedProduct?.map((Product) => <div className="cursor-pointer  group overflow-hidden relative shadow-xl p-2" key={Product._id}>
        ...
        <button onClick={() => addToCartProduct(Product._id) } className="group-hover:translate-y-0  translate-y-[310%]   hover:bg-green-500 hover:text-white  border border-green-400 rounded-md w-full py-1.2 my-5 cursor-pointer transition-all duration-200">add To cart</button>
